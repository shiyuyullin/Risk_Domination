		...
		while (!inputFileStream.eof())
		{
			...
			if (temp == "[continents]")
			{
				...
				while (temp != "[countries]")
				{
					...
					resultMap->setContinent(i, tempContinent); //Using setters for map
					numberOfContinents++;
					i++;
				}
			}
			...
			if (temp == "[countries]")
			{
				...
				while (serialNumS != "[borders]")
				{
					...
					resultMap->setCountry(k, tempCountry);
					numberOfCountries++;
			...
			resultMap->setNumberOfCountries(numberOfCountries); //FOR TESTING
			int count = -1;
			if (serialNumS == "[borders]")
			{
				while (!inputFileStream.eof())
				{
					...
					while (row >> border)
					{
						...
						resultMap->addEdge(count, border);
